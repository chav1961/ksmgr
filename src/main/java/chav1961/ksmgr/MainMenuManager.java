...
class MainMenuManager extends JEnableMaskManipulator {
	...
	private static final Template[]	TEMPLATES = {
										...
										new Template((m)->m.currentSelection == SelectedWindows.LEFT && m.leftRepo && m.leftRepoSelected || m.currentSelection == SelectedWindows.RIGHT && m.rightRepo && m.rightRepoSelected,
												EDIT | EDIT_RENAME | EDIT_DELETE | TASKS | TASKS_KEYS | TASKS_KEYS_EXPORT | TASKS_KEYS_IMPORT),
										new Template((m)->(m.currentSelection == SelectedWindows.LEFT || m.currentSelection == SelectedWindows.RIGHT) && m.leftRepo && m.rightRepo && (m.leftRepoSelected || m.rightRepoSelected),
												EDIT | EDIT_COPY | EDIT_MOVE),
										new Template((m)->m.currentSelection == SelectedWindows.LEFT && m.leftRepo && !m.leftRepoSelected || m.currentSelection == SelectedWindows.RIGHT && m.rightRepo && !m.rightRepoSelected,
